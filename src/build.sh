...
if [[ "$AUTO_DEVOPS_BUILD_IMAGE_CNB_ENABLED" != "false" && ! -f Dockerfile && -z "${DOCKERFILE_PATH}" ]]; then
  ...
  pack config registry-mirrors add 'registry-1.docker.io' --mirror https://docker-io.cluster.cnative.dev -q
  pack config registry-mirrors add 'gcr.io' --mirror https://gcr-io.cluster.cnative.dev -q
  pack config registry-mirrors add 'ghcr.io' --mirror https://ghcr-io.cluster.cnative.dev -q
  pack config registry-mirrors add 'quay.io' --mirror https://quay-io.cluster.cnative.dev -q
